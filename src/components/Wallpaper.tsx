...
//
// The pattern's own coordinate space is 1100x1100, and the root <svg>
// has no viewBox, so 1 pattern unit == 1 real css pixel — bigger than
// most phone viewports, so without correction you only ever see a
// slice of one oversized repeat ("zoomed in", sparse). `patternTransform
// ="scale(0.32)"` shrinks the effective tile to ~350px so several
// repeats tile per screen, WhatsApp-wallpaper style, while keeping
// every icon's relative hero-to-micro size hierarchy intact. Base
// opacity is also kept low (text-ink/[0.045]) so the texture stays a
// backdrop and never competes with real message bubbles.
//
...
export function Wallpaper() {
  return (
    <svg
      aria-hidden="true"
      className="pointer-events-none absolute inset-0 h-full w-full text-ink/[0.045]"
